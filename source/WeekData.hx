...
class WeekData {
	...
	public static var storyType:String = '';
	...
	public static function reloadWeekFiles(isStoryMode:Null<Bool> = false)
	{
		...
		var storyList:Array<String> = CoolUtil.coolTextFile(Paths.getPreloadPath('weeks/storyList.txt'));

		if (inFreeplay) {
			for (k in 0...storyList.length) {
				var sexList:Array<String> = CoolUtil.coolTextFile(Paths.getPreloadPath('weeks/' + storyList[k] + '/weekList.txt'));
				for (i in 0...sexList.length) {
					for (j in 0...directories.length) {
						var fileToCheck:String = directories[j] + 'weeks/' + storyList[k] + '/' + sexList[i] + '.json';
			...
			var sexList:Array<String> = CoolUtil.coolTextFile(Paths.getPreloadPath('weeks/' + storyType + '/weekList.txt'));
			for (i in 0...sexList.length) {
				for (j in 0...directories.length) {
					var fileToCheck:String = directories[j] + 'weeks/' + storyType + '/' + sexList[i] + '.json';
					if(!weeksLoaded.exists(sexList[i])) {
						var week:WeekFile = getWeekFile(fileToCheck);
						if(week != null) {
							var weekFile:WeekData = new WeekData(week, sexList[i]);
